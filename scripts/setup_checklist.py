...
def check_env():
    ...
    try: importlib.import_module(pkg); items.append((f"pkg:{pkg}",True))
    except: items.append((f"pkg:{pkg}",False))
    for f in ["rel_vref.txt","rel_extract.txt","eng_extract.txt"]:
    items.append((f"data/{f}", (BASE/"data"/f).exists()))
    passed=sum(1 for _,ok in items if ok); pct=passed/len(items)*100
    print("\nStatus check:")
